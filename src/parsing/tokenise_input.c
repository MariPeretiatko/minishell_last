...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/22 22:31:16 by mperetia          #+#    #+#             */
/*   Updated: 2024/04/22 22:31:17 by mperetia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	define_token_type(char **cmd, int **cmd_type, int i,
				bool *cmd_in_pipe);
		...
		bool *cmd_in_pipe)
{
	...
	else if ((i > 0 && (*cmd_type)[i - 1] == CMD_NAME) || (i > 0
			&& (*cmd_type)[i - 1] == CMD_ARG) || *cmd_in_pipe == true)
		(*cmd_type)[i] = CMD_ARG;
	else
	{
		(*cmd_type)[i] = CMD_NAME;
