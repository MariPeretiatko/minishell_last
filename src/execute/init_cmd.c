...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/22 22:30:05 by mperetia          #+#    #+#             */
/*   Updated: 2024/04/22 22:30:05 by mperetia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../inc/minishell.h"

static char	**get_split_path(t_envp *env);
...
static char	**get_split_path(t_envp *env) {
  ...
  link = env;
  while (link && ft_strcmp(link->name, "PATH"))
    link = link->next;
  split_path = NULL;
  if (link)
    split_path = ft_split(link->value, ':');
  return (split_path);
}

static int	get_arg_nb(int *tokens);

char	**get_cmd_args(char **cmd_arg, int *tokens, t_envp *env, char *cmd) {
  char **args;
  int nb_arg;
  int idx;
