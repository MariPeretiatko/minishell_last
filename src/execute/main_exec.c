...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/22 22:30:18 by mperetia          #+#    #+#             */
/*   Updated: 2024/04/22 22:30:18 by mperetia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../inc/minishell.h"

t_execute	*exec_init(char *cmd, t_envp *env);
char		***init_cmd_arr(t_execute *exec, char **cmd_split);
int			**init_tokens_arr(t_execute *exec, int save);
void		get_last_cmd_stat(t_execute *exec, int idx, char *path,
				char **args);
static void	launch_exec(t_execute *exec);
static void	cmd_exec(char *path, char **args, int cmd_nb, t_execute *exec);
static void	cmd_loop(t_execute *exec, int cmd, char *cmd_path, char **cmd_args);
static void	wait_children(int *cpid, int cmds_cnt);

void	main_execute(char *cmd, t_envp **env)
{
	t_execute	*exec;
	int			i;

	exec = exec_init(cmd, *env);
...
static void	cmd_loop(t_execute *exec, int cmd, char *cmd_path, char **cmd_args)
{
	while (exec->cmds[cmd])
	{
		...
		{
			cmd_path = get_cmd_path(exec->cmds[cmd], exec->toks[cmd],
					*exec->env);
			cmd_args = get_cmd_args(exec->cmds[cmd], exec->toks[cmd],
					*exec->env, cmd_path);
...
static void	cmd_exec(char *path, char **args, int cmd_nb, t_execute *exec)
{
	...
	if (exec->pipe_fd[(cmd_nb + 1) % 2][0] == -1 || exec->pipe_fd[cmd_nb
		% 2][1] == -1 || !args || check_dir(args[0]))
		return ;
	if (path == NULL && args[0])
	{
