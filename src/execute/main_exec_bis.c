...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/22 22:30:12 by mperetia          #+#    #+#             */
/*   Updated: 2024/04/22 22:30:12 by mperetia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_last_cmd_stat(t_execute *exec, int idx, char *path, char **args)
{
	if (!path && !is_builtin(exec->cmds[idx], exec->toks[idx]) && args)
		g_exit_status = 127;
	if (exec->pipe_fd[idx % 2][1] == -1 || exec->pipe_fd[(idx + 1) % 2][0] ==
		-1)
		g_exit_status = 1;
